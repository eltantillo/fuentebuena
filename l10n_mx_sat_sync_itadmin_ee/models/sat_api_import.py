...
class SAT:
    ...
    def sign(self, esignature_cer_bin, solicitud):
        ...
        if hasattr(crypto, 'sign'):
            signed_info = base64.b64encode(crypto.sign(self.private_key, element_to_sign, 'sha1')).decode(
                "UTF-8").replace("\n", "")
        else:
            crypto_key = self.private_key.to_cryptography_key()
            signed_info = base64.b64encode(crypto_key.sign(element_to_sign, padding.PKCS1v15(), hashes.SHA1())).decode(
                "UTF-8").replace("\n", "")
